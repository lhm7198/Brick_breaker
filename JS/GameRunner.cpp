...
//----------------
void Game::receiveKeyinputs1(){
	float p1_paddle_speed;
	float p2_paddle_speed;
	if(event.type == sf::Event::KeyPressed){
		switch(event.key.code){
			case sf::Keyboard::A:
				if(p1_paddle[0].get_Paddle_x() > 0){
					p1_paddle_speed = p1_paddle[0].get_Paddle_speedX();
					p1_paddle[0].move(-p1_paddle_speed, 0);
				}
				break;
			case sf::Keyboard::D:
				if(p1_paddle[0].get_Paddle_x() < screen_width - p1_paddle[0].get_Paddle_width()){
					p1_paddle_speed = p1_paddle[0].get_Paddle_speedX();
					p1_paddle[0].move(p1_paddle_speed, 0);
				}
				break;
